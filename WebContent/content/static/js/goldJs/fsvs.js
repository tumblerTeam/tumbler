	...
	$.fn.fsvs = function( options ) {
		...
		var mouseWheelHandler = function( ev ) {
			...
			if( isChrome() ) {
				...
				wheely = Math.floor( wheely / 3 );
			}
			if( ( ! scrolling || ( options.mouseWheelDelay && Date.now() > mouseWheelScrollStart + options.mouseWheelDelay ) ) && Math.abs( wheely ) > 3 ) {
		...
		var isScrollingUp = function(ev){
			...
			var wheely = ( e.wheelDelta || -e.detail || e.originalEvent.detail );
			var delta = Math.max( -1, Math.min( 1, wheely ) );
			if( isChrome() ) wheely = Math.floor( wheely / 3 );
